...
- 單號自動編 `R<年月日>-序號`
- **顧客送修的日期與時間自動記下來**，顯示成 `9/6（日）14:30`，旁邊附「5 小時前」。
  客人昨天送來、今天才開單的話，開單畫面上改一下就好
- **每個階段各蓋一個實際時間戳**：收件 → 報價 → 開修 → 完工 → 取件，
  按綠色按鈕推進時自動記錄，卡片上排成一條時間軸並算出總歷時，完全不用手填
- **超過預計完成時間自動紅框、排到最上面**
- 電話是可點的，手機上直接撥
- 待取件的單一鍵產生取件通知話術（會帶上送修日期與維修歷時）；報價中的單一鍵產生報價說明
- 搜尋支援日期，打 `9/5` 就找得到那天送修的單
- 統計：待取件（含逾期未取）、店內在修、今日進單、**今日毛利**

**v2 新增：收機的那三分鐘**

- **現況照片**：拍完直接存，瀏覽器端先壓到最長邊 1280、JPEG 0.7 才上傳。
  存實體檔在 `photos/`，資料庫只記檔名。爭議時這是唯一的證據
- **收機檢查表**：IMEI／序號、開機密碼、一起收的配件、客戶是否已備份、機內資料是否重要
- **免責確認**：泡水、進水、主機板、開不了機這幾類，**沒簽就推不進「維修中」**，會跳視窗擋下來
- **保固追蹤**：取件當下自動算保固到期日，天數可改（泡水機只保 7 天）。
  搜尋電話或姓名時直接顯示「保固中（剩 42 天）」或「已過保」
- **成本與訂金**：填成本就即時算毛利並用顏色標示；收訂金後待取件會顯示「尚需收款」
- **客戶歷史**：電話打滿 8 碼自動查同一支電話的舊單，一鍵沿用上次姓名與機型
- **料件庫存**（第三個分頁）：常換的螢幕電池建進去，開單點一下帶入成本，
  推到維修中時問你要不要扣一片，低於安全存量在接單頁提醒叫料
- **取件簽收**：完成時填取件人（可改成「客戶友人 王小姐」）與實收金額
- **未取件提醒**：修好放超過 7 天沒人拿，卡片標紅並置頂
- **列印標籤**：單號、客戶、機型、收機時間、IMEI、配件，附 Code 39 條碼（條碼槍掃得到）

**手機也能用**：程式現在綁 `0.0.0.0`，啟動時會印出區網網址，
例如 `手機請連 http://192.168.1.23:8420`。收機拍照就是拿手機拍。
公用網路請把 `config.json` 的 `lan_access` 改成 `false`，或用 `--local-only` 啟動。
